...
const onMemTimer = (options) => {
  stats.getMemory((err, memData) => {
    ...
    if (memPercent < options.memory && thresholdExceeded.memory) {
      log(['ops', 'memory', 'normal', 'threshold'], `Memory use of ${memPercent}% has dropped below threshold of ${options.memory}%`);
...
const onDiskTimer = (options) => {
  stats.getDisk(options, (err, result) => {
    ...
    if (spacePercent < options.space && thresholdExceeded.space) {
      log(['ops', 'disk', 'normal', 'threshold'], `Disk use of ${spacePercent}% has dropped below threshold of ${options.space}%`);
...
const onCPUTimer = (options) => {
  ...
  if (options['cpu-five-minutes'] > 0 && oneMinLoad < options['cpu-five-minutes'] && thresholdExceeded.cpu_five_minutes) {
    log(['ops', 'cpu', 'warning', 'threshold'], `Average 5-min CPU load of ${fiveMinLoad.toFixed(3)}, exceeds threshold of ${options['cpu-five-minutes']}`);
  ...
  if (options['cpu-fifteen-minutes'] > 0 && oneMinLoad < options['cpu-fifteen-minutes'] && thresholdExceeded.cpu_fifteen_minutes) {
    log(['ops', 'cpu', 'warning', 'threshold'], `Average 15-min CPU load of ${fifteenMinLoad.toFixed(3)}, exceeds threshold of ${options['cpu-fifteen-minutes']}`);
...
const onInodeTimer = (options) => {
  stats.getInodes(options.partition, (err, partitionName, inodeUseage) => {
    ...
    if (inodeUseage < options.inode && thresholdExceeded.inode) {
      log(['ops', 'inodes', 'normal', 'threshold'], `Inode use of ${inodeUseage}% has dropped below threshold of ${options.inode}%`);
...
module.exports = (options) => {
  ...
  log(['ops', 'info'], {
    ...
    logInterval: options.reportRate,
