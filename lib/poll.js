...
const onCPUTimer = (options) => {
  ...
  if (options.cpu_one_minute && oneMinLoad > options.cpu) {
    log(['ops', 'cpu', 'warning', 'threshold'], `Average 1-min CPU load of ${oneMinLoad.toFixed(3)}, exceeds threshold of ${options.cpu}`);
  }
  if (options.cpu_five_minute && fiveMinLoad > options.cpu) {
    log(['ops', 'cpu', 'warning', 'threshold'], `Average 5-min CPU load of ${fiveMinLoad.toFixed(3)}, exceeds threshold of ${options.cpu}`);
  }
  if (options.cpu_fifteen_minute && fifteenMinLoad > options.cpu) {
    log(['ops', 'cpu', 'warning', 'threshold'], `Average 15-min CPU load of ${fifteenMinLoad.toFixed(3)}, exceeds threshold of ${options.cpu}`);
  }
};
