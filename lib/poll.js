...
const onCPUTimer = (options) => {
  ...
  if (options.mi === 1 && oneMinLoad > options.cpu) {
    log(['ops', 'cpu', 'warning', 'threshold'], `Average 1-min CPU load of ${oneMinLoad.toFixed(3)}, exceeds threshold of ${options.cpu}`);
  }
  if (options.mi === 5 && fiveMinLoad > options.cpu) {
    log(['ops', 'cpu', 'warning', 'threshold'], `Average 5-min CPU load of ${oneMinLoad.toFixed(3)}, exceeds threshold of ${options.cpu}`);
  }
  if (options.mi === 1 && fifteenMinLoad > options.cpu) {
    log(['ops', 'cpu', 'warning', 'threshold'], `Average 15-min CPU load of ${oneMinLoad.toFixed(3)}, exceeds threshold of ${options.cpu}`);
  }
};
