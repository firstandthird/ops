...
const onMemTimer = (options) => {
  stats.getMemory((err, memData) => {
    ...
    if (options.verbose) {
      log(['memory', 'info'], `Using ${memPercent}% of memory`);
    }
    if (memPercent > options.memory) {
      log(['memory', 'warning', 'threshold'], `Using ${memPercent}% of memory, exceeds threshold of ${options.memory}%`);
      thresholdExceeded.memory = true;
    }
    if (memPercent < options.memory && thresholdExceeded.memory) {
      log(['memory', 'restored', 'threshold'], `Memory use of ${memPercent}% has dropped below threshold of ${options.memory}%`);
...
const onDiskTimer = (options) => {
  stats.getDisk(options, (err, result) => {
    ...
    if (options.verbose) {
      log(['disk', 'info'], `Using ${spacePercent}% of available disk space`);
    }
    if (spacePercent > options.space) {
      log(['disk', 'warning', 'threshold'], `Using ${spacePercent}% of disk space, exceeds threshold of ${options.space}%`);
      thresholdExceeded.space = true;
    }
    if (spacePercent < options.space && thresholdExceeded.space) {
      log(['disk', 'restored', 'threshold'], `Disk use of ${spacePercent}% has dropped below threshold of ${options.space}%`);
...
const onCPUTimer = (options) => {
  ...
  if (options['cpu-one-minute'] > 0 && oneMinLoad > options['cpu-one-minute']) {
    log(['cpu', 'warning', 'threshold'], `Average 1-min CPU load of ${oneMinLoad.toFixed(3)}, exceeds threshold of ${options['cpu-one-minute']}`);
    thresholdExceeded.cpu_one_minute = true;
  }
  if (options['cpu-five-minute'] > 0 && fiveMinLoad > options['cpu-five-minute']) {
    log(['cpu', 'warning', 'threshold'], `Average 5-min CPU load of ${fiveMinLoad.toFixed(3)}, exceeds threshold of ${options['cpu-five-minute']}`);
    thresholdExceeded.cpu_five_minutes = true;
  }
  if (options['cpu-fifteen-minute'] > 0 && fifteenMinLoad > options['cpu-fifteen-minute']) {
    log(['cpu', 'warning', 'threshold'], `Average 15-min CPU load of ${fifteenMinLoad.toFixed(3)}, exceeds threshold of ${options['cpu-fifteen-minute']}`);
  ...
  if (options['cpu-one-minute'] > 0 && oneMinLoad < options['cpu-one-minute'] && thresholdExceeded.cpu_one_minute) {
    log(['cpu', 'restored', 'threshold'], `Average 1-min CPU load of ${oneMinLoad.toFixed(3)}, has dropped back to normal range`);
    thresholdExceeded.cpu_one_minute = false;
  }
  if (options['cpu-five-minutes'] > 0 && fiveMinLoad < options['cpu-five-minutes'] && thresholdExceeded.cpu_five_minutes) {
    log(['cpu', 'restored', 'threshold'], `Average 5-min CPU load of ${fiveMinLoad.toFixed(3)}, has dropped back to normal range`);
    thresholdExceeded.cpu_five_minutes = false;
  }
  if (options['cpu-fifteen-minutes'] > 0 && fifteenMinLoad < options['cpu-fifteen-minutes'] && thresholdExceeded.cpu_fifteen_minutes) {
    log(['cpu', 'restored', 'threshold'], `Average 15-min CPU load of ${fifteenMinLoad.toFixed(3)}, has dropped back to normal range`);
